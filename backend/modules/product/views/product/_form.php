...
<?php    if ($model->isNewRecord) { $model->tax=1;} ?>
                ...
                <div class='col-md-4 col-sm-6 col-xs-12 '>
                    <div class="form-group field-product-search_tag">
                        <label class="control-label" for="product-search_tag">Search Tag </label><label onclick="jQuery('#modal-7').modal('show', {backdrop: 'fade'});" class="btn btn-icon btn-white tag_cat">Select Tag Category</label>
                    <?php
                    if (!$model->isNewRecord) {
                ...
                <div class='col-md-6 col-sm-6 col-xs-12 '>
                    <?= $form->field($model, 'profile')->fileInput()->label('profile Picture<i> (400*450)</i>') ?>
                    <?php
                    if (!$model->isNewRecord) {
                ...
                <div class='col-md-6 col-sm-6 col-xs-12 '>
                    <?= $form->field($model, 'other_image[]')->fileInput(['multiple' => true, 'accept' => 'image/*'])->label('Gallery Images<i> (400*450)</i>') ?>
                    <?php if (!$model->isNewRecord) { ?>
                                                                                                                                                                                                                    <!--<a href=''><img src="<?= yii::$app->homeUrl ?>/../../uploads/product/1/dasda_0.jpg" width="100" alt="Delete"></a>-->
                        <div class="row">
                            <?php
                            $path = Yii::getAlias('@paths') . '/product/' . $model->id . '/gallery_large';
                                ...
                                foreach (glob("{$path}/*") as $file) {
                                    ...
                                    if ($img_nmees['1'] != '') {
                                        ...
                                        <div class = "col-md-2 img-box" id="<?= $k; ?>">
                                            <a href="<?= Yii::$app->homeUrl . '../uploads/product/' . $model->id . '/gallery_large/' . end($arry) ?>" target="_blank">
                                                <img src="<?= Yii::$app->homeUrl . '../uploads/product/' . $model->id . '/gallery_large/' . end($arry) ?>" width="50px" height="50px"></a>
...
$('#product-product_name').blur(function () {
			var description_text = "Buy " + this.value + " from Carnation City Perfumes, Dubai's top perfume manufacturing company.";
